...
export class GetMessagesService {
  ...
  run(send: boolean) {
    return send
      ? send.toString() == 'true'
        ? this.messagePrismaRepository.getAllSentMessages()
        : this.messagePrismaRepository.getAllReceivedMessages()
      : this.messagePrismaRepository.getAllMessages();
  }

  obtainSentReceived(send: boolean) {
    if (send.toString() == 'true') {
      console.log('SEND IGUAL TRUE');
      return this.messagePrismaRepository.getAllSentMessages();
    } else {
      console.log('SEND IGUAL FALSE');
      return this.messagePrismaRepository.getAllReceivedMessages();
    }
  }
