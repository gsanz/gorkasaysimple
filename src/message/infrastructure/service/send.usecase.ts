...
export class SendService {
  ...
  async run(createMessageDto: CreateMessageDto) {
    if (createMessageDto.type == 'template') {
      throw new BadRequestException('You can not send a Template Message');
    }

    return this.mappertoOutInComingMessage(
      await this.messagePrismaRepository.create(
